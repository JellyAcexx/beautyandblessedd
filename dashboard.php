...
.top-section {
    ...
    justify-content: space-between;
    align-items: stretch;
    gap: 26px; /* or gap: 20px; */
}

.top-customer-box {
    flex: 0 0 340px;         /* fixed min width */
    max-width: 420px;        /* adjust as needed */
    width: 100%;
}

.top-product-box {
    flex: 1 1 0;             /* gets remaining space */
    max-width: 900px;        /* adjust as needed */
    width: 100%;
    /* REMOVE margin: 0 auto; ito ang dahilan kung bakit nagce-center */
}
...
<div class="header-container">
    ...
    <form method="GET" class="filter-form">
        <select name="month" id="filterSelect">
        ...
        <select name="year" id="filterSelect">
...
<div class="content-scroll" style="max-height:calc(100vh - 80px); overflow-y:auto; padding:15px;">
<?php if($totalData==0): ?>
    <div style="
        max-width: 450px;
        margin: 40px auto;
    ...
    ">
        <h3 style="color:#c32f59; font-weight:700; margin-bottom:10px;">
            <i class="fa-solid fa-circle-info" style="margin-right:8px;"></i>
            No Data Available
        </h3>

        <p style="color:#6d2e3a; margin-bottom:20px; font-size:1.05em;">
            There is no data for the selected month and year.
        </p>
...
<div class="top-product-box" style="width:100%;max-width:800px;">
    ...
    <?php $rank=1; foreach($top_products as $p): ?>
      ...
      ">
        <!-- Image: white bg, full contain, no crop -->
            <?php
            $img = $p['image_path'] ?? '';
            if (!str_starts_with($img, 'pictures/')) {
                $img = 'pictures/' . $img;
            }
...
var noData = <?php echo ($totalData==0 ? 'true' : 'false'); ?>;
