...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Select from admin_login_tb
    $sql = "SELECT email, password FROM admin_login_tb WHERE email='$email' LIMIT 1";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // INSERT INTO admin_forgot_tb (only columns: email, code, password, date_time)
        $insert = $conn->prepare("INSERT INTO admin_forgot_tb (email, code, password, date_time) VALUES (?, ?, ?, ?)");
        $insert->bind_param("siss", $email, $code, $oldPassword, $date_time);
        $ok = $insert->execute();
        $insert->close();

        if (!$ok) {
            $error = "Could not create reset request. Please try again.";
        } else {
