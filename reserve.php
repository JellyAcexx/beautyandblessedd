...
try {
    ...
    if (!empty($outOfStock)) {
        $conn->rollback();
        echo json_encode(['success' => false, 'message' => "Insufficient stock for: " . implode(', ', $outOfStock)]);
        exit;
    }
    ...
    $sql_summary = "
        SELECT r.reservation_date, r.pickup_date, r.total, 
               CONCAT(u.register_fname, ' ', u.register_lname) AS customer_name, u.register_email, u.register_fname, u.register_lname, u.register_id
        FROM reservations r
    ...
    $to = $summary_result['register_email'];
    $subject = "Reservation Received";
    $message = "Hi " . $summary_result['register_fname'] . ",\n\nYour reservation has been received successfully! Your reservation is pending and is valid for 3 days (until " . date("F d, Y", strtotime($summary_result['pickup_date'])) . ").\n\nReservation ID: $reservation_id\nTotal: ₱" . number_format($summary_result['total'],2) . "\n\nThank you for choosing us!";
    $headers = "From: reservationsystem@beautyandblessed.online\r\n";
    mail($to, $subject, $message, $headers);

    // Insert notification for customer (notifcustomer)
    $notif_customer_sql = "INSERT INTO notifcustomer (register_id, notif_message, notif_type, notif_link)
        VALUES (?, ?, ?, ?)";
    $notif_customer_stmt = $conn->prepare($notif_customer_sql);
    $notif_message = "Reservation made! Your reservation (ID: $reservation_id) is pending for 3 days. Pickup on " . date("F d, Y", strtotime($summary_result['pickup_date'])) . ".";
    $notif_type = "reservation";
    $notif_link = "reservation.php";
    $notif_customer_stmt->bind_param("isss", $register_id, $notif_message, $notif_type, $notif_link);
    if (!$notif_customer_stmt->execute()) {
        error_log("Notifcustomer insert error: " . $notif_customer_stmt->error);
    }
    $notif_customer_stmt->close();

    // Admin in-app notification (notifadmin)
    $admin_id = 1;
    $notif_sql = "INSERT INTO notifadmin (register_id, notif_message, notif_type, notif_link)
                  VALUES (?, ?, ?, ?)";
    $notif_stmt = $conn->prepare($notif_sql);
    $notif_message = "New reservation by {$summary_result['register_fname']} {$summary_result['register_lname']}, ID: $reservation_id.";
    $notif_type = 'reservation';
    $notif_link = 'reservations.php';
    $notif_stmt->bind_param("isss", $admin_id, $notif_message, $notif_type, $notif_link);
    if (!$notif_stmt->execute()) {
        error_log("Admin notification insert error: " . $notif_stmt->error);
    }
    $notif_stmt->close();

    // 🌸 Send SMTP HTML email to admin (via PHPMailer)
    $admin_email = "detorresjanellemae@gmail.com";
    $admin_subject = "New Reservation Received";
    $admin_message_html = "
        <h2>New Reservation</h2>
        <p>
            By: <b>{$summary_result['register_fname']} {$summary_result['register_lname']}</b><br>
            Email: {$summary_result['register_email']}
        </p>
        <p>Reservation ID: <b>$reservation_id</b><br>
           Pickup Date: " . date("F d, Y", strtotime($summary_result['pickup_date'])) . "<br>
           Total: ₱" . number_format($summary_result['total'],2) . "</p>
        <br>
        <p><a href='https://beautyandblessed.online/admin/reservations.php'>View in Admin Panel</a></p>
    ";
    ...
    try {
        ...
        $mailAdmin->Subject = $admin_subject;
        $mailAdmin->Body    = $admin_message_html;
        $mailAdmin->send();
    } catch (Exception $e) {
        error_log("PHPMailer admin send error: " . $mailAdmin->ErrorInfo);
    }
